...
Display( )
{
	...
	if (CurrentView == OUTSIDE) {
		...
		// Inside view always uses perspective
		gluPerspective(70.f, 1.f, 0.1f, 1000.f);
	}
	...
	if (CurrentView == OUTSIDE) {
		...
		// No rotation or scaling in Inside view
	}
	...
	// draw the object by calling up its display list:
	...
	for (int i = 0; i < NUM_HORSES; i++) {
		glPushMatrix();
			...
			glRotatef(HThetaX[i].GetValue(nowSecs), 1., 0., 0.);
			glRotatef(HThetaY[i].GetValue(nowSecs), 0., 1., 0.);
			glRotatef(HThetaZ[i].GetValue(nowSecs), 0., 0., 1.);
			glTranslatef(HXpos[i].GetValue(nowSecs), HYpos[i].GetValue(nowSecs), HZpos[i].GetValue(nowSecs));

			glCallList(SolidHorseLists[i]);
		glPopMatrix();
	}
...
InitGraphics( )
{
	...
	for (int i = 0; i < NUM_HORSES; i++) {
		float t = 10.0f - i * 1.0f;  // each horse finishes within 10 seconds

		HYpos[i].Init();
		HYpos[i].AddTimeValue(0.0f, 0.0f);
		HYpos[i].AddTimeValue(t * 0.125f, 0.5f);
		HYpos[i].AddTimeValue(t * 0.25f, 0.0f);
		HYpos[i].AddTimeValue(t * 0.375f, -0.5f);
		HYpos[i].AddTimeValue(t * 0.5f, 0.0f);
		HYpos[i].AddTimeValue(t * 0.625f, 0.5f);
		HYpos[i].AddTimeValue(t * 0.75f, 0.0f);
		HYpos[i].AddTimeValue(t * 0.875f, -0.5f);
		HYpos[i].AddTimeValue(t, 0.0f);

		HThetaY[i].Init();
		HThetaY[i].AddTimeValue(0.0, 0.0);
		HThetaY[i].AddTimeValue(10.0f, -360);

		// Step 4: Rocking back and forth (pitching)
		HThetaX[i].Init();
		HThetaX[i].AddTimeValue(0.0f, 0.0f);
		HThetaX[i].AddTimeValue(t * 0.25f, 20.0f);   
		HThetaX[i].AddTimeValue(t * 0.5f, 0.0f);
		HThetaX[i].AddTimeValue(t * 0.75f, -20.0f);  
		HThetaX[i].AddTimeValue(t, 0.0f);
	}
...
void DrawCircle() {
	...
	for (int i = 0; i < segments; ++i) {
		...
		glVertex3f(x, 0.0f, z); // Circle in XZ plane
	}
	glEnd();
}


void DrawHorse(GLuint listID, float angleDeg) {
	glNewList(listID, GL_COMPILE);
	glPushMatrix();

	// Rotate horse to its position around the ring
	glRotatef(angleDeg, 0.f, 1.f, 0.f);

	// Move horse outward to ring radius
...
InitLists( )
{
	...
	for (int i = 0; i < NUM_HORSES; i++) {
		...
		DrawHorse(SolidHorseLists[i], angleDeg);
	}
...
InitMenus( )
{
	...
	glutAddMenuEntry("Outside View", OUTSIDE);
	glutAddMenuEntry("Inside View", INSIDE);
