...
GLSLProgram Snake; // your VS+FS shader program name
...
void Display()
{
	...
	Snake.Use(); // turns the Snake shader program on
	float pigd = 9.0f - Time * (9.0f + 13.0f); 
	float pigh = 3.f * (1.0f - Time);       
	// Set uniform variables for the shader
	Snake.SetUniformVariable("uPigD", pigd);
	Snake.SetUniformVariable("uPigH", pigh);
...
InitGraphics()
{
	...
	//LoadTexture("BMP/snake.bmp", &SnakeTex);
	Snake.Init();
	bool valid = Snake.Create("PigInPython.vert", "PigInPython.frag");
	if (!valid)
	{
		fprintf(stderr, "Yuch!The Snake shader did not compile.\n");
	}
	else
	{
		fprintf(stderr, "Woo - Hoo!The Snake shader compiled.\n");
	}
...
void BuildSnakeList()
{
	//static GLuint s_SnakeGeo = 0;
	//if (s_SnakeGeo == 0)
	//{
	//	s_SnakeGeo = LoadObjMtlFiles((char*)"SnakeH.obj");
	//	if (s_SnakeGeo == 0)
	//	{
	//		fprintf(stderr, "Failed to load Snake.obj\n");
	//	}
	//}
	...
	////glTranslatef(1.2f, 1.5f, -2.7f);
	//SetMaterial(0.70f, 0.25f, 0.85f, 100.f);
	//glCallList(s_SnakeGeo);
	//glPopMatrix();
	//glEnable(GL_TEXTURE_2D);
	//glEndList();

	GLuint s_SkelGeo = LoadObjMtlFiles((char*)"SnakeH.obj");
	SnakeList = glGenLists(1);
	glNewList(SnakeList, GL_COMPILE);
	SetMaterial(1.0f, 1.0f, 1.0f, 50.0f);
